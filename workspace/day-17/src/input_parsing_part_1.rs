use std::fs;
use crate::part_1::{SimulationContext, ActiveCube};
...
###".to_string();
        ...
        assert!(context.activated_cubes.contains(&ActiveCube { x: 1, y: 0, z: 0 }));
        assert!(context.activated_cubes.contains(&ActiveCube { x: 2, y: 1, z: 0 }));
        assert!(context.activated_cubes.contains(&ActiveCube { x: 0, y: 2, z: 0 }));
        assert!(context.activated_cubes.contains(&ActiveCube { x: 1, y: 2, z: 0 }));
        assert!(context.activated_cubes.contains(&ActiveCube { x: 2, y: 2, z: 0 }));
    }
}